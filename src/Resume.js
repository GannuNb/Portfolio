...
/*
  Canvas -> PNG slices -> jsPDF => clickable link annotations.
  Fixed: explicit page handling + robust coordinate mapping so links are clickable.
*/
...
export default function Resume() {
    ...
    // selectors for the 5 contact link elements (data-pdf-link attributes)
    ...
    const generatePdfWithLinks = async ({ openInNewTab = true } = {}) => {
        ...
        try {
            // Ensure web fonts loaded
            if (document.fonts && document.fonts.ready) await document.fonts.ready;
            await new Promise((r) => setTimeout(r, 120));

            // Render resume to a single high-res canvas
            ...
            // Gather link elements and their href + bounding rect *relative to resumeRef*
            const linkEntries = [];
            for (const sel of contactLinkSelectors) {
                const el = resumeRef.current.querySelector(sel);
                if (!el) continue;
                const rootRect = resumeRef.current.getBoundingClientRect();
                ...
                // Only store valid hrefs
                if (!href) continue;
                // push DOM-space entries (unscaled)
                linkEntries.push({
                    href,
                    leftDomPx,
                    topDomPx,
            ...
            // Helpful: function converting canvas px -> PDF pts
            // pxPerPt = canvasWidthPx / A4_WIDTH_PTS  => 1 pt = pxPerPt canvas px
            ...
            while (yOffsetCanvasPx < canvasHeightPx) {
                ...
                // For page selection: jsPDF already has first page, add new for subsequent
                if (pageIndex === 0) {
                    pdf.setPage(1);
                } else {
                    pdf.addPage();
                ...
                // Place image on the page: x=0, y = top of page (0)
                // jsPDF uses top-left origin for addImage coordinates with 'pt' units
                pdf.addImage(dataUrl, "PNG", 0, 0, imgWidthPts, imgHeightPts, undefined, "FAST");

                // Add link annotations that intersect this slice
                // Each link entry DOM -> convert to canvas px (scaled by CANVAS_SCALE)
                for (const entry of linkEntries) {
                    ...
                    // If entry is outside this slice, skip
                    ...
                    // X coordinate in canvas px (left is same across slices)
                    const leftCanvasPx = entryLeftCanvasPx;
                    const visibleTopWithinSliceCanvasPx = visibleTopCanvasPx - sliceTopCanvasPx;

                    // Convert to PDF pts
                    const pdfX = canvasPxToPdfPts(leftCanvasPx); // from left edge
                    const pdfY = canvasPxToPdfPts(visibleTopWithinSliceCanvasPx); // from top of page
                    ...
                    if (pdfW > 0 && pdfH > 0 && entry.href) {
                        try {
                            // jsPDF.link expects (x, y, w, h, { url })
                            pdf.link(pdfX, pdfY, pdfW, pdfH, { url: entry.href });
                        } catch (err) {
                            // some viewers / situations may throw, but we keep going
